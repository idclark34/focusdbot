...
class BotModel: ObservableObject {
    ...
    private func tick() {
        ...
        // Allow by exact bundle id or by common app name fallbacks (Safari variants)
        var allowed = (currentBundle == selfBundle) || sessionAllowedBundles.contains(currentBundle) || globalAllowed.contains(currentBundle)
        if !allowed {
            if currentBundle == "Safari" && globalAllowed.contains("com.apple.Safari") { allowed = true }
            if currentBundle == "Safari Technology Preview" && globalAllowed.contains("com.apple.SafariTechnologyPreview") { allowed = true }
        }

        var reallyAllowed = allowed

        // Check website rules for Safari ONLY if Safari is not already allowed as an app.
        let isSafari = (currentBundle == "com.apple.Safari" || currentBundle == "Safari")
        let isSafariTP = (currentBundle == "com.apple.SafariTechnologyPreview" || currentBundle == "Safari Technology Preview")
        if (isSafari || isSafariTP) && !allowed {
